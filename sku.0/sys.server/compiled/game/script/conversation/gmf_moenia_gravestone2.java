...
public class gmf_moenia_gravestone2 extends script.base_script
{
    public gmf_moenia_gravestone2()
    {
    }
    public static final String c_stringFile = "conversation/gmf_moenia_gravetone2";
    ...
    public boolean grave2_defaultCondition(obj_id player, obj_id npc) throws InterruptedException
    {
        return true;
    }
    //The player has completed the scavenger hunt
    public boolean grave2_taskCondition(obj_id player, obj_id npc) throws InterruptedException
    {
        if (!hasObjVar(player, "the_spectre.quest_state"))
        {
            return false;
        }
        return getIntObjVar(player, "the_spectre.quest_state") == 2;
    }
    ...
    public boolean grave2_onQuestCondition(obj_id player, obj_id npc) throws InterruptedException
    {
        //Are they on the quest at all?
        return hasObjVar(player, "the_spectre.quest_state");
    }
    /*
        This branch is our intro conversation.
        So this is the first dialog shown to the player
    */
    public int grave2_handleBranch1(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("player_investigate"))
        {
            ...
            setObjVar(player, "the_spectre.quest_state", 1);
            return SCRIPT_CONTINUE;
        }
        return SCRIPT_DEFAULT;
    }
    public int grave2_handleBranch2(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("player_i_need_help_finding_stuff"))
        {
            ...
            utils.setScriptVar(player, "conversation.grave2.branchId", 3);
    ...
    public int grave2_handleBranch3(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
    ...
    public int grave2_handleBranch4(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("player_offer_tribute"))
        {
            ...
            npcEndConversationWithMessage(player, message);
            setObjVar(player, "the_spectre.quest_state", 3);
            return SCRIPT_CONTINUE;
        }
        return SCRIPT_DEFAULT;
    }
    public int OnAttach(obj_id self) throws InterruptedException
    {
        ...
        setName(self, "Spooky Gravestone");
        return SCRIPT_CONTINUE;
    }
    ...
    public int OnStartNpcConversation(obj_id npc, obj_id player) throws InterruptedException
    {
        ...
        else if (grave2_haunted(player, npc))
        {
            ...
            //Set the branch that we want to go to, to check for the above responses
            utils.setScriptVar(player, "conversation.grave2.branchId", 2);

            npcStartConversation(player, npc, "grave2", message, responses);
        }
        else if (grave2_taskCondition(player, npc))
        {
            string_id message = new string_id(c_stringFile, "npc_released");
            int numberOfResponses = 1;
            int responseIndex = 0;
            ...
            utils.setScriptVar(player, "conversation.grave2.branchId", 4);
            npcStartConversation(player, npc, "grave2", message, responses);
        }
        else if (grave2_onQuestCondition(player, npc))
        {
        ...
        else if (grave2_defaultCondition(player, npc))
        {
            ...
            utils.setScriptVar(player, "conversation.grave2.branchId", 1);
    ...
    {
        if (!conversationId.equals("grave2"))
        {
            return SCRIPT_CONTINUE;
        }
        int branchId = utils.getIntScriptVar(player, "conversation.grave2.branchId");
        if (branchId == 1 && grave2_handleBranch1(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 2 && grave2_handleBranch2(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 3 && grave2_handleBranch3(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 4 && grave2_handleBranch4(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        chat.chat(npc, "Error:  Fell through all branches and responses for OnNpcConversationResponse.");
        utils.removeScriptVar(player, "conversation.grave2.branchId");
        return SCRIPT_CONTINUE;
    }
}