...
// ----------------------------------------------------------------------
	...
	{
        faceTo(npc, player);
        if (groundquests.isTaskActive(player, "quest/goto_camp_czerka", "gotoCampCzerka"))
        {
            return true;
        }
        return false;
...
// ----------------------------------------------------------------------
	...
	{
        if (groundquests.isQuestActive(player, "quest/goto_camp_czerka"))
        {
            return true;
        }
        return false;
...
// ----------------------------------------------------------------------
	...
	{
		//-- [BRANCH NOTE] 
		//-- NPC: Unfortunately, everyone has something to inherit. 

		//-- [RESPONSE NOTE] 
		//-- PLAYER: Indeed they do.
		if (response.equals("s_6"))
		{
			//-- [NOTE] 
			if (meghan_lorso_condition__defaultCondition (player, npc))
			{
				//-- NPC: By chance, did you want to work for Czerka?
				string_id message = new string_id (c_stringFile, "s_7");
				int numberOfResponses = 0;
				
				boolean hasResponse = false;
				
				//-- PLAYER: I could always use something new to occupy my time. What kind of work does Czerka need help doing?
				boolean hasResponse0 = false;
				if (meghan_lorso_condition__defaultCondition (player, npc))
				{
					++numberOfResponses;
...
// ----------------------------------------------------------------------
	...
	public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
	{
		...
		if (branchId == 5 && meghan_lorso_handleBranch4 (player, npc, response) == SCRIPT_CONTINUE)
			return SCRIPT_CONTINUE;
		
		if (branchId == 6 && meghan_lorso_handleBranch4 (player, npc, response) == SCRIPT_CONTINUE)
			return SCRIPT_CONTINUE;
		
		if (branchId == 7 && meghan_lorso_handleBranch4 (player, npc, response) == SCRIPT_CONTINUE)
			return SCRIPT_CONTINUE;
		
		if (branchId == 8 && meghan_lorso_handleBranch4 (player, npc, response) == SCRIPT_CONTINUE)
			return SCRIPT_CONTINUE;
