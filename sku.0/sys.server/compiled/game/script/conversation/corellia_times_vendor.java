...
public class corellia_times_vendor extends script.base_script {
    public static final String c_stringFile = "conversation/corellia_times_vendor";
    ...
    public int corellia_times_vendor_handleBranch1(obj_id self, obj_id player, string_id response) throws InterruptedException {
        if (response.equals("s_2")) {
            corellia_times_vendor_action_showTokenVendorUI(player, self);
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException {
        ...
        if (corellia_times_vendor_condition__defaultCondition(player, self)) {
            prose_package pp = new prose_package();
            pp = prose.setStringId(pp, new string_id(c_stringFile, "s_1"));
            pp.target.set(player);
            npcStartConversation(player, self, "corellia_times_vendor", null, pp, new string_id[]{new string_id(c_stringFile, "s_2"), new string_id(c_stringFile, "s_4"), new string_id(c_stringFile, "s_6")});
            return SCRIPT_CONTINUE;
        }
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException {
        ...
        if (corellia_times_vendor_handleBranch1(self, player, response) == SCRIPT_CONTINUE) {
            return SCRIPT_CONTINUE;
        }
