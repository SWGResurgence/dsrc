...
public class resource extends script.base_script
{
    ...
    {
        if (params == null || params.isEmpty() || !utils.hasScriptVar(self, SCRIPTVAR_INUSE))
        {
            cleanup();
            return SCRIPT_CONTINUE;
        }
        switch (sui.getIntButtonPressed(params))
        {
            case sui.BP_OK:
                ...
                if (isIdValid(resourceChosen))
                {
                    ...
                    int amount = RESOURCE_AMOUNT + hasObjVar(self, "resource.supply_generator") ? 20000 : 0;
                    obj_id crate = createResourceCrate(resourceChosen, amount, utils.getInventoryContainer(player));
                    if (isIdValid(crate))
                    {
                        setObjVar(self, OBJVAR_RESOURCE_REWARDED, true);
                        messageTo(self, "handleVeteranHarvestDestroy", null, 1, false);
                        CustomerServiceLog("veteran", "Giving player %TU " + amount + " of resource type " + resourceChosen + ". They have TWICE confirmed that they wish to consume this deed in order to get a crate of their chosen resource.", player);
                    }
                    cleanup();
                }
                else 
