    ...
    public void calcAndSetPrototypeProperties(obj_id prototype, draft_schematic.attribute[] itemAttributes) throws InterruptedException
    {
        ...
        for (draft_schematic.attribute itemAttribute : itemAttributes)
        {
            ...
            if (!calcAndSetPrototypeProperty(prototype, itemAttribute))
            {
                ...
                {
                    space_crafting.setWeaponMaximumDamage(prototype, itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("damage_min"))
                {
                    space_crafting.setWeaponMinimumDamage(prototype, itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("effective_shields"))
                {
                    space_crafting.setWeaponShieldEffectiveness(prototype, itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("effective_armor"))
                {
                    space_crafting.setWeaponArmorEffectiveness(prototype, itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("energy_per_shot"))
                {
                    space_crafting.setWeaponEnergyPerShot(prototype, itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("refire_rate"))
                {
                    space_crafting.setWeaponRefireRate(prototype, itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("projectile_speed"))
                {
                    space_crafting.setWeaponProjectileSpeed(prototype, itemAttribute.currentValue);
                }
                else
                {
