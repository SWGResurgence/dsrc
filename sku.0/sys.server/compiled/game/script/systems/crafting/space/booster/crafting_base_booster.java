    ...
    public void calcAndSetPrototypeProperties(obj_id prototype, draft_schematic.attribute[] itemAttributes) throws InterruptedException
    {
        ...
        for (draft_schematic.attribute itemAttribute : itemAttributes)
        {
            ...
            if (!calcAndSetPrototypeProperty(prototype, itemAttribute))
            {
                ...
                {
                    space_crafting.setBoosterMaximumEnergy(prototype, itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("booster_recharge"))
                {
                    space_crafting.setBoosterEnergyRechargeRate(prototype, itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("booster_consumption"))
                {
                    space_crafting.setBoosterEnergyConsumptionRate(prototype, itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("booster_speed"))
                {
                    space_crafting.setBoosterMaximumSpeed(prototype, itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("booster_acceleration"))
                {
                    space_crafting.setBoosterAcceleration(prototype, itemAttribute.currentValue);
                }
