    ...
    public void calcAndSetPrototypeProperties(obj_id prototype, draft_schematic.attribute[] itemAttributes) throws InterruptedException
    {
        ...
        for (draft_schematic.attribute itemAttribute : itemAttributes)
        {
            ...
            if (!calcAndSetPrototypeProperty(prototype, itemAttribute))
            {
                ...
                {
                    setObjVar(prototype, "fltMinDamage", itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("fltMaxDamage") || ((itemAttribute.name).getAsciiId()).equals("fltMaxEffectiveness"))
                {
                    setObjVar(prototype, "fltMaxDamage", itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("fltRefireRate"))
                {
                    setObjVar(prototype, "fltRefireRate", itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("fltShieldEffectiveness"))
                {
                    setObjVar(prototype, "fltShieldEffectiveness", itemAttribute.currentValue);
                }
                if (((itemAttribute.name).getAsciiId()).equals("fltArmorEffectiveness"))
                {
                    setObjVar(prototype, "fltArmorEffectiveness", itemAttribute.currentValue);
                }
